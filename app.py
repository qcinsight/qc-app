...
# Optional PDF builder (guarded so the app still works if it's missing)
try:
...
if use_demo or uploaded:
    ...
    st.dataframe(ctl_summary, use_container_width=True)
    ...
    st.subheader("Plots")
    if p1.exists():
        st.image(str(p1), caption="Metric by Group (auto-chosen)", use_container_width=True)
    if p2.exists():
        st.image(str(p2), caption="External Controls Heatmap (if available)", use_container_width=True)
